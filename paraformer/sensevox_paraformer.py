...
# ------------------- Configuration -------------------
# --- MODIFIED: Changed model directory base for Paraformer ---
MODEL_DIR_BASE = "paraformermodel"
# -----------------------------------------------------------
...
CHANNELS = 1
RATE = 16000 # Sherpa Paraformer model expects 16kHz
# ---------------------------
...
def update_status_display(page: ft.Page, lv: ft.ListView):
    ...
    if is_capturing_hotkey: status_val = "Status: Press desired hotkey..."; status_col = ft.Colors.BLUE_500
    elif is_listening: status_val = f"Status: Listening for '{current_hotkey}' (Paraformer/PyAudio)"; status_col = ft.Colors.GREEN_500
        ...
        else: status_val = "Status: Stopped (Paraformer/PyAudio Ready)"; status_col = ft.Colors.ORANGE_500
...
def start_listening(e: ft.ControlEvent):
    ...
    if model is None:
        # --- MODIFIED: Load Paraformer model ---
        log(page, lv, "Loading Sherpa-ONNX Paraformer recognizer...", "INFO")
        load_start = time.perf_counter()
        try:
            model = sherpa_onnx.OfflineRecognizer.from_paraformer(
                paraformer=MODEL_FILE_PATH,
...
def language_chip_selected(e: ft.ControlEvent):
    ...
    # Paraformer model does not use language setting, but we keep the UI logic.
    log(page, lv, "NOTE: Paraformer model does not use this language setting.", "WARNING")
    if language_chip_row.current:
        changed = False
...
def main(page: ft.Page):
    ...
    lang_chips = [ft.Chip(label=ft.Text(lang.upper()), data=lang, selected=(lang == current_selected_language_value), tooltip=f"Select '{lang}' (Note: Not used by Paraformer)", on_select=language_chip_selected, disabled=True) for lang in LANG_OPTIONS]
...
if __name__ == "__main__":
    ...
    print("[INFO] Starting Flet application (Paraformer/PyAudio, Hotkey Blocking)...")
